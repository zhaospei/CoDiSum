...
from pygments.lexers import JavaLexer, PythonLexer, CppLexer, CLexer, JavascriptLexer, CSharpLexer
...
def line_filter_cmt(line, language="java"):
    if language == "java" or language == 'c' or language == "cpp" or language == "csharp" or language == "javascript":
...
class Data4CopynetV3:
    def __init__(self):
        ...
        self.time = []
        # save the raw commit id
        self.id = []

    def build(self, filenames, path = None, txt_file = False, need_time = False):
        ...
        lan_cache_path = os.path.join(path, "cache")
        diff_file_lst, msg_file_lst, lang_file_list = filenames
        data = list()
        for i in range(len(diff_file_lst)):
            ...
            lang_per_file = open(lang_file_list[i],"r").read().split("\n")
        ...
        logger.info("{} commits in {} are None; {} commits are in Chinese".format(count_none, lang, count_ch))
    ...
    def save_data(self, path, save_difftext=False, save_msgtext=False, save_diff=False, save_msg=False,
        ...
        json.dump(self.id, open(os.path.join(path, 'id.json'), 'w'))
        if save_difftext:
    ...
    def load_data(self, path, load_difftext=True, load_msgtext=True, load_diff=True, load_msg=True,
        ...
        # load data from disk
        self.id = json.load(open(os.path.join(path, 'id.json')))
        if load_difftext:
    ...
    def data_constraint(self, min_diff, max_diff, min_msg, max_msg):
        na, nb, nc, nd, ne, nf, ng, ni = [], [], [], [], [], [], [], []
        for i, j, k, l, m, n, o, id in zip(self.diffmark, self.difftoken, self.msg, self.variable, self.difftext,
                                    self.msgtext, self.diffatt, self.id):
            if min_diff <= len(i) < max_diff and min_msg <= len(k) < max_msg:
                na.append(i), nb.append(j), nc.append(k), nd.append(l), ne.append(m), nf.append(n), ng.append(o), ni.append(id)
    ...
    def remove_unk(self): # [delete]
        na, nb, nc, nd, ne, nf, ng, ni = [], [], [], [], [], [], [], []
        lemmatization = json.load(open('lemmatization.json'))
        for i, j, k, l, m, n, p, id in zip(self.diffmark, self.difftoken, self.msg, self.variable, self.difftext,
                                    self.msgtext, self.diffatt, self.id):
            is_unk = False
            for o in k:
                ...
                if (o.lower() not in self.word2index and o not in l) or o.lower() == 'flaw' or o.lower() == 'flaws':
                    ...
                    break
            if not is_unk:
                na.append(i), nb.append(j), nc.append(k), nd.append(l), ne.append(m), nf.append(n), ng.append(p), ni.append(id)
        self.diffmark, self.difftoken, self.msg, self.variable, self.difftext, self.msgtext, self.id = na, nb, nc, nd, ne, nf, ni
        self.diffatt = ng
...
if __name__ == '__main__':
    ...
    parser.add_argument("-rl","--repo_list", metavar="repo_list.txt", default = None, # ".../repo_list.json",
    ...
    parser.add_argument("-in","--input_folder", metavar="raw_data/", default = None, # default=".../raw_data"
                        help='the folder which contains raw data include diff files and msg files', required = False)
    ...
    repo_full_name = list()
    repo_list = open(args.repo_list, "r").read().split("\n")
    # for lan in args.language:
    ...
    for repo in repo_list:
        diff_list.append(os.path.join(args.input_folder, repo + '.diff'))
        msg_list.append(os.path.join(args.input_folder, repo + '.msg'))
        lang_list.append(os.path.join(args.input_folder, repo + '.lang'))
    
    repo_full_name.append(diff_list)
    repo_full_name.append(msg_list)
    repo_full_name.append(lang_list)
    ...
    output_folder = os.path.join(args.output_folder, "sort_{}-min_diff{}-max_diff{}-min_msg{}-max_diff{}".format(args.sort, args.min_diff, args.max_diff, args.min_msg, args.max_msg))
